...
namespace Zurhay
{
    ...
    //    public int thisDayDate = 0;
      ...
      //  public int thisYear =0;
       ...
       // readonly static Uri remoteUri = new Uri("http://zurhay.ru/editor/uploads/download.php?XMLName=give&userid=3");

       // public string fileName = "ms-banner.gif", myStringWebResource = null;
    ...
    private void CheckZurhayNewXML()
        ...
        {
            // создаем запрос на скачивание нового обновления Скачиваем новую xml
            ...
            //тупо качаем без запроса
         //   try
          //  {
                //Асинхронное скачивание(фоновое)
                /*needchangexml = 1;
                //качаем календарь
                WebClient webClient = new WebClient();
                webClient.Encoding = Encoding.UTF8;
                ...
                // webClient.DownloadFile(url, documentsPathtoDownload + localDownloadFilename);
                webClient.DownloadStringCompleted += (s, e) => {
                    var text = e.Result; // get the downloaded text
                    string localPathToDownload = System.IO.Path.Combine(documentsPathtoDownload, localDownloadFilename);
                    File.WriteAllText(localPathToDownload, text); // writes to local storage
                    ...
                    if (File.Exists(System.IO.Path.Combine(documentsPathtoDownload, localDownloadFilename)))
                    {
                        Toast.MakeText(this, "Календарь обновился", ToastLength.Short).Show();
            ...
            // Finish(); //пока оставлю финишь. надо сделать
                ...
                //     string filename = System.IO.Path.GetFileName(url.Loca);
                ...
                webClient.DownloadFile(url, localPathToDownload);
